...
# sys.path.append("..")
from Env.SupplementforEnv.obstacle2 import Obstacle

obsMap = Obstacle()
from Env.SupplementforEnv.agent import Agent
...
class Env:
    ...
    def init_env(self, num_agents, obstacle_map):
        ...
        for ndx in ndxs[:num_agents]:
            agents.append(Agent(obsMap.nodes[obsMap.patrol_nodes[ndx]][0] + 0.5,
                                obsMap.nodes[obsMap.patrol_nodes[ndx]][1] + 0.5))  # 输入随机点的坐标。这里可以换一下，换成索引里点的坐标
        #            agents.append(Agent())
    ...
    def step(self, node_list, node_last_list, last_vertex, state):
        ...
        for indx, agent_pos in enumerate(agent_pos_list):
            self.current_map_state = self.vsb.update_visibility_get_local(agent_pos, agent_g_pos_list[indx],
                                                                          self.current_map_state, self.vsbPoly,
                                                                          self.patrol_points)  # 假设这个括号里的current_map_state就是只算巡逻点的
        ...
        for i in range(len(self.patrol_points[0])):
            ...
            if self.current_map_state[
                obsMap.nodes[obsMap.patrol_nodes[i]][0], obsMap.nodes[obsMap.patrol_nodes[i]][1]] == 255:
                Env.visits[i] += 1
                visit_flag = True
    ...
    def reset(self):
        ...
        self.node_last_list = []
        for i in range(CONST.NUM_AGENTS):
            self.node_last_list.append(
                obsMap.get_key([agent_pos_list[i][0] - 0.5, agent_pos_list[i][1] - 0.5], obsMap.nodes))
