...
class AttentionModel(nn.Module):
    ...
    def __init__(self, env, args, tanh_clipping=10, normalization='batch', checkpoint_encoder=False):
        super(AttentionModel, self).__init__()
        encoder_dim = args.num_hidden
    ...
    def forward(self, encoder_output, node_last_list, decision_index, decision_flag, agent_id, shift_action=None):

        decoder_input = encoder_output

        log_prob, selected, shift_action_total = self._get_log_p(encoder_output,
                                                                 self.adj, node_last_list, agent_id, decision_flag,
                                                                 shift_action)
    ...
    def _get_log_p(self, decoder_input, adj, node_last_list, agent_id, decision_flag, shift_action, normalize=True):
        ...
        decision_flag = decision_flag.reshape(batch_size, CONST.NUM_AGENTS)
        visitflag = [False for i in range(len(obsMap.patrol_nodes))]
        nodes_last_goal = shift_action[0, 0, :, :]
        for i in range(CONST.NUM_AGENTS):
            if not decision_flag[0][i]:
                a = get_key(nodes_last_goal[i], obsMap.nodes)
                if a == None:
                    continue
                k = obsMap.patrol_nodes.index(a)
                visitflag[k] = True
                continue
        ...
        for agent in range(CONST.NUM_AGENTS):
            ...
            if agent < CONST.NUM_AGENTS - 1 and not istrain:
                ...
                for i in range(batch_size):
                    if decision_flag[i][agent]:
                        shift_action[i, agent + 1, agent + 1, :] = torch.tensor(obsMap.nodes
                                                                                [obsMap.patrol_nodes[
                                selected[i].clone()]]).to(device)
                        c = 1
                    else:
                        log_prob[i, :] = -1e8
                        selected[i] = obsMap.patrol_nodes.index(
                            get_key(shift_action[i, agent, agent + 1, :], obsMap.nodes))

            # shift_action=shift_action.contiguous()

            if istrain and agent < CONST.NUM_AGENTS - 1:
                log_prob = log_prob.clone()
                selected = selected.reshape(batch_size)
                for i in range(batch_size):
                    if not decision_flag[i][agent]:
                        log_prob[i, :] = -1e8
                        a = get_key(shift_action[i, agent, agent + 1, :], obsMap.nodes)
                        selected[i] = obsMap.patrol_nodes.index(a)

            log_prob = log_prob.reshape(batch_size, graph_size)
        ...
        log_prob_total, action_total = self.adjust_Seq(action_total, log_prob_total, agent_id)

        return log_prob_total, action_total, shift_action

    def adjust_Seq(self, action_total, log_prob_total, agent_id):  # all tensors

        # agent_id : (batch_size,num_agents)
        # action_total : (batch_size,num_agents)
        # log_prob_total: (batch_size,num_agents,graph_size)
        act_total = torch.zeros_like(action_total).to(device)
        log_p_total = torch.zeros_like(log_prob_total).to(device)

        batch_size = act_total.shape[0]
        agent_id = agent_id.reshape(batch_size, CONST.NUM_AGENTS)
        for b in range(batch_size):
            for agent in range(CONST.NUM_AGENTS):
                act_total[b, agent_id[b][agent]] = action_total[b, agent]
                log_p_total[b, agent_id[b][agent], :] = log_prob_total[b, agent, :]

        return log_p_total, act_total
...
class Attention_(nn.Module):
    ...
    def forward(self, query, key, value):
        ...
        K = self.key_(key).view(B, L, self.n_head, G, D // self.n_head).transpose(1, 2).transpose(2,
                                                                                                  3)  # (B,nh,L,G,hdim)
        V = self.value_(value).view(B, L, self.n_head, G, D // self.n_head).transpose(1, 2).transpose(2,
                                                                                                      3)  # (B,nh,G,L,hdim)
        Q = self.query_(query).view(B, L, self.n_head, G, D // self.n_head).transpose(1, 2).transpose(2,
                                                                                                      3)  # (B,nh,G,L,hdim)
...
class SelfAttention_(nn.Module):
    ...
    def forward(self, query, key, value):  # (batch_size,num_agent,hidden_dim)
        ...
        K = self.ln(key + self.key_(key)).view(B, L, self.n_head, D // self.n_head).transpose(1,
                                                                                              2)  # (B,nh,L,hdim) L=n_agents
        V = self.ln(value + self.value_(value)).view(B, L, self.n_head, D // self.n_head).transpose(1,
                                                                                                    2)  # (B,nh,L,hdim)
        Q = self.ln(query + self.query_(query)).view(B, L, self.n_head, D // self.n_head).transpose(1,
                                                                                                    2)  # (B,nh,1,hdim)
