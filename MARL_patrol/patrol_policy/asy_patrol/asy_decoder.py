...
class AttentionModel(nn.Module):
    ...
    def forward(self,encoder_output,node_last_list,decision_index,decision_flag,agent_id,shift_action=None):
        ...
        log_prob,selected,shift_action_total=self._get_log_p(encoder_output,
                                                             self.adj,node_last_list,agent_id,decision_flag,shift_action)
              ...
              if agent<CONST.NUM_AGENTS-1 and not istrain:
                  ...
                  for i in range(batch_size):
                      if decision_flag[i][agent]:
                          ...
                          log_prob[i,:]=-1e8
                          selected[i]=obsMap.patrol_nodes.index(get_key(shift_action[i,agent,agent+1,:],obsMap.nodes))
...
class Attention_(nn.Module):
    ...
    def forward(self,query,key,value):
        ...
        K=self.key_(key).view(B,L,self.n_head, G,D//self.n_head).transpose(1,2).transpose(2,3) # (B,nh,L,G,hdim)
